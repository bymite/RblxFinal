...
// Store hostname->IP mappings so we can send hostname in SOCKS5
#define MAX_HOST_CACHE 256
...
// Hook getaddrinfo to cache hostname->IP mappings
static int (*orig_getaddrinfo)(const char *, const char *, const struct addrinfo *, struct addrinfo **);
static int hook_getaddrinfo(const char *hostname, const char *servname, const struct addrinfo *hints, struct addrinfo **res) {
    ...
    if (result == 0 && hostname && res && *res) {
        ...
        while (ai) {
            if (ai->ai_family == AF_INET) {
                ...
                cache_host(s->sin_addr.s_addr, hostname);
            }
            ai = ai->ai_next;
        }
    }
        ...
        [UIView animateWithDuration:0.4 animations:^{ banner.alpha = 1.0; }
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 3 * NSEC_PER_SEC),
                ...
                [UIView animateWithDuration:0.4 animations:^{ banner.alpha = 0; }
                                 completion:^(BOOL d) { [banner removeFromSuperview]; }];
            });
        }];
    });
}
...
static int socks5_handshake(int sockfd, const char *dest_host, int dest_port) {
    ...
    // Send CONNECT with hostname (type 0x03)
...
static int hook_connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
    ...
    // Skip if connecting to proxy itself
    if (memcmp(&s->sin_addr.s_addr, &proxy_ip, 4) == 0)
        return orig_connect(sockfd, addr, addrlen);

    // Look up cached hostname for this IP
